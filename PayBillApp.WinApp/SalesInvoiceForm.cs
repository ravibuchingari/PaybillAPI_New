...
namespace PayBillApp.WinApp
{
    public partial class SalesInvoiceForm : Form
    {
        ...
        {
            if (ConstantVariables.CLIENT_ID == 0 || ConstantVariables.JWT_TOKEN == null || ConstantVariables.JWT_TOKEN.Length == 0)
                Environment.Exit(0);
            else
            {
                ...
                lblClientName.Text = ConstantVariables.CLIENT_NAME;
                lblInvoiceDate.Text = DateTime.Now.ToString("dd-MMM-yyyy");
                chkBarcodeOnly.Checked = barcodeScanOnly;
                ChkBarcodeOnly_Click(null, null);
                try
                {
                    _ = new SqliteDataEngine();
                    GetUtilities();
                }
                catch (Exception ex)
                {
        ...
        private void BtnExit_Click(object sender, EventArgs e)
        {
            ...
            {
                ReuseMethods.SerializeApplicationVariables(barcodeScanOnly);
                this.Close();
            }
        }
